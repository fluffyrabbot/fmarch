...
test("admin route data exposes local core loop proof as a native audit row", async () => {
  ...
  assert.equal(coreLoop.status, "4 core loop lanes passed");
  ...
  assert.deepEqual(
    ...
    ["core-loop", "action-loop", "player-action-boundary", "private-channel"],
  );
...
test("admin route data exposes local seed fixture summary as a native audit row", async () => {
  ...
  assert.equal(seed.status, "9 demo scenarios available locally");
...
test("admin local seed fixture detail data carries scenario rows", async () => {
  ...
  assert.equal(data.audit.scenarios.length, 9);
  assert.deepEqual(
