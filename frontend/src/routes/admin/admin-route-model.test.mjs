...
test("admin route data exposes local seed fixture summary as a native audit row", async () => {
  ...
  assert.equal(seed.status, "83 demo scenarios available locally");
...
test("admin local seed fixture detail data carries scenario rows", async () => {
  ...
  assert.equal(data.audit.scenarios.length, 83);
  assert.deepEqual(
    ...
    [
      ...
      ["stale-cohost-deadline-reconnect-recovery", "available_locally"],
...
function seedFixtureSummaryFixture() {
  return {
    ...
    demoScenarios: [
      ...
      seedScenario(
        "stale-cohost-deadline-reconnect-recovery",
        "Stale cohost deadline reconnect",
        "cohost",
      ),
      seedScenario(
        "concurrent-host-deadline-advance-race",
