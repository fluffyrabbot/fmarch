...
test("admin identity lifecycle detail data carries audit event rows", async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      ["account_recovery_credential_issued", "host_h", "host_h"],
...
test("admin route data exposes hosted identity evidence as a native audit row", async () => {
  ...
  assert.deepEqual(identity.hostedHandoffChecklist.blockedReceipt, {
    ...
    rawEvidenceContractSummary:
      hostedIdentityBlockedReceipt.rawEvidenceContractSummary,
    ...
    blockedOperatorPacket: hostedIdentityBlockedReceipt.blockedOperatorPacket,
    ...
    [
      [
        ...
        [
          ...
          "selectedProductionFeatureRoleUrl /_dev/ops/audit/local-core-loop?game=<seeded-game>",
          "recoveryCommand npm run test:dev-test-game-hosted-evidence-lane",
          "proofTarget target/dev-test-game/hosted-evidence-lane.json",
          "packetProofTarget target/dev-test-game/hosted-target-preflight.json",
          "nextProofTarget target/dev-test-game/hosted-evidence-lane.json",
...
test("admin route data exposes local core loop proof as a native audit row", async () => {
  ...
  assert.deepEqual(coreLoop.artifactSummary, {
    ...
    completedGameCoverageLaneCount: 11,
...
test("admin route data exposes local identity adapter proof as a native audit row", async () => {
  ...
  assert.deepEqual(identity.artifactSummary, {
    ...
    accountRecovery: {
      ...
      recoveryRoleUrl:
        "/auth/account/recovery?account=host%40example.test&returnTo=%2Fg%2Fgame-a%2Fhost",
