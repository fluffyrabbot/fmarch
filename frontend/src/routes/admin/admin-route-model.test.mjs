...
test("admin route data exposes local hardening proof as a native audit row", async () => {
  ...
  assert.equal(hardening.status, "75 hardening lanes passed");
...
test("admin local hardening detail data carries lane rows", async () => {
  ...
  assert.equal(data.audit.checks.length, 75);
  assert.deepEqual(
    data.audit.checks.map((check) => [check.id, check.status]),
    [
      ...
      ["replacement-stale-session-after-refresh", "passed"],
      ["replacement-reconnect-recovery", "passed"],
      ["replacement-stale-conflict-message", "passed"],
      ...
      ["concurrent-cohost-deadline-resolve-race", "passed"],
      ["concurrent-cohost-deadline-resolve-race-reload", "passed"],
      ["concurrent-replacement-private-post-race", "passed"],
      ["concurrent-replacement-private-post-race-reload", "passed"],
      ["concurrent-replacement-vote-race", "passed"],
      ["concurrent-replacement-vote-race-reload", "passed"],
      ["concurrent-replacement-action-race", "passed"],
      ["concurrent-replacement-action-race-reload", "passed"],
      ["replacement-incoming-action", "passed"],
      ["replacement-action-reconnect", "passed"],
      ["replacement-stale-action-after-resolve", "passed"],
      ["replacement-stale-private-post-after-resolve", "passed"],
      ["replacement-stale-private-post-reconnect", "passed"],
      ["replacement-stale-private-post-after-complete", "passed"],
      ["replacement-stale-private-post-after-complete-reload", "passed"],
      ["concurrent-host-publish-race", "passed"],
      ...
      ["stale-host-complete-reload", "passed"],
      [
        "stale-host-complete-reconnect-recovery",
        "passed: reconnecting -> recovered, completed true",
      ],
      ["concurrent-host-complete-race", "passed"],
      ["concurrent-host-complete-race-reload", "passed"],
      ...
      ["concurrent-host-resolve-race-reload", "passed: locked true, routes 200/200"],
      ["concurrent-host-advance-race", "passed"],
      ["concurrent-host-advance-race-reload", "passed"],
      ["concurrent-host-deadline-advance-race", "passed"],
      ["concurrent-host-deadline-advance-race-reload", "passed"],
      ["concurrent-host-lifecycle-race", "passed"],
...
test("admin route data exposes local seed fixture summary as a native audit row", async () => {
  ...
  assert.equal(seed.status, "80 demo scenarios available locally");
...
test("admin local seed fixture detail data carries scenario rows", async () => {
  ...
  assert.equal(data.audit.scenarios.length, 80);
  assert.deepEqual(
    data.audit.scenarios.map((scenario) => [scenario.id, scenario.status]),
    [
      ["host-phase-controls", "available_locally"],
      ["cohost-deadline-control", "available_locally"],
      ["player-vote-recovery", "available_locally"],
      ["player-action-denied", "available_locally"],
      ["invalid-action-recovery", "available_locally"],
      ["resolution-receipt", "available_locally"],
      ["dead-player-recovery", "available_locally"],
      ["night-action-loop", "available_locally"],
      ["action-idempotent-retry", "available_locally"],
      ["concurrent-action-race", "available_locally"],
      ["concurrent-action-race-reload", "available_locally"],
      ["concurrent-vote-race-reload", "available_locally"],
      ["concurrent-player-vote-resolve-race", "available_locally"],
      ["concurrent-player-vote-resolve-race-reload", "available_locally"],
      ["concurrent-player-action-advance-race", "available_locally"],
      ["concurrent-player-action-advance-race-reload", "available_locally"],
      ["concurrent-cohost-deadline-resolve-race", "available_locally"],
      ["concurrent-cohost-deadline-resolve-race-reload", "available_locally"],
      ["concurrent-replacement-private-post-race", "available_locally"],
      ["concurrent-replacement-private-post-race-reload", "available_locally"],
      ["concurrent-replacement-vote-race", "available_locally"],
      ["concurrent-replacement-vote-race-reload", "available_locally"],
      ["concurrent-replacement-action-race", "available_locally"],
      ["concurrent-replacement-action-race-reload", "available_locally"],
      ["replacement-incoming-action", "available_locally"],
      ["replacement-action-reconnect", "available_locally"],
      ["replacement-stale-action-after-resolve", "available_locally"],
      ["replacement-stale-private-post-after-resolve", "available_locally"],
      ["replacement-stale-private-post-reconnect", "available_locally"],
      ["replacement-stale-private-post-after-complete", "available_locally"],
      [
        "replacement-stale-private-post-after-complete-reload",
        "available_locally",
      ],
      ["concurrent-host-resolve-race", "available_locally"],
      ["concurrent-host-resolve-race-reload", "available_locally"],
      ["stale-host-resolve-reload", "available_locally"],
      ["stale-host-resolve-reconnect-recovery", "available_locally"],
      ["concurrent-host-advance-race", "available_locally"],
      ["concurrent-host-advance-race-reload", "available_locally"],
      ...
      ["concurrent-host-lifecycle-race", "available_locally"],
      ["concurrent-host-lifecycle-race-reload", "available_locally"],
      ["concurrent-host-complete-race", "available_locally"],
      ["concurrent-host-complete-race-reload", "available_locally"],
      ["stale-host-prompt-reload", "available_locally"],
      ["stale-host-complete-reload", "available_locally"],
      ["stale-host-complete-reconnect-recovery", "available_locally"],
      ["concurrent-player-complete-race", "available_locally"],
      ["public-player-complete-reload", "available_locally"],
      ["stale-player-complete-reload", "available_locally"],
      ["concurrent-host-mixed-advance-race", "available_locally"],
      ["concurrent-host-mixed-advance-race-reload", "available_locally"],
      ["stale-same-action-recovery", "available_locally"],
      ["stale-action-conflict-message", "available_locally"],
      ["stale-action-reconnect-recovery", "available_locally"],
      ...
      ["replacement-stale-session-after-refresh", "available_locally"],
      ["replacement-reconnect-recovery", "available_locally"],
      ["replacement-stale-conflict-message", "available_locally"],
      ["replacement-invalid-target-recovery", "available_locally"],
      ["replacement-idempotent-retry", "available_locally"],
      ["stale-host-invite-recovery", "available_locally"],
      ["replacement-stale-success-recovery", "available_locally"],
      ["replacement-stale-player", "available_locally"],
      ["replacement-stale-action", "available_locally"],
      ["replacement-stale-private-channel", "available_locally"],
      ["replacement-stale-private-receipts", "available_locally"],
...
function seedFixtureSummaryFixture() {
  return {
    ...
    demoScenarios: [
      ...
      seedScenario("cohost-deadline-control", "Cohost deadline control", "cohost"),
      seedScenario("player-vote-recovery", "Player vote recovery", "player"),
      seedScenario("player-action-denied", "Player action denied", "player"),
      seedScenario("invalid-action-recovery", "Invalid action recovery", "actionPlayer"),
      seedScenario("resolution-receipt", "Resolution receipt", "deniedPlayer"),
      seedScenario("dead-player-recovery", "Dead player recovery", "deniedPlayer"),
      ...
      seedScenario("concurrent-action-race-reload", "Concurrent action reload", "actionPlayer"),
      seedScenario("concurrent-vote-race-reload", "Concurrent vote reload", "player"),
      seedScenario(
        "concurrent-player-vote-resolve-race",
        "Concurrent player vote/resolve race",
        "player",
      ...
      seedScenario(
        "concurrent-player-vote-resolve-race-reload",
        "Concurrent player vote/resolve reload",
        "player",
      ),
      seedScenario(
        "concurrent-player-action-advance-race",
      ...
      seedScenario(
        "concurrent-player-action-advance-race-reload",
        "Concurrent player action/advance reload",
        "actionPlayer",
      ),
      seedScenario(
        "concurrent-cohost-deadline-resolve-race",
      ...
      seedScenario(
        "concurrent-cohost-deadline-resolve-race-reload",
        "Concurrent cohost deadline/resolve reload",
        "cohost",
      ),
      seedScenario(
        "concurrent-replacement-private-post-race",
      ...
      seedScenario(
        "concurrent-replacement-private-post-race-reload",
        "Concurrent replacement private-post reload",
        "player",
      ),
      seedScenario(
        "concurrent-replacement-vote-race",
      ...
      seedScenario(
        "concurrent-replacement-vote-race-reload",
        "Concurrent replacement vote reload",
        "player",
      ),
      seedScenario(
        "concurrent-replacement-action-race",
      ...
      seedScenario(
        "concurrent-replacement-action-race-reload",
        "Concurrent replacement action reload",
        "player",
      ),
      seedScenario("replacement-incoming-action", "Incoming replacement action", "player"),
      seedScenario(
        "replacement-action-reconnect",
        "Replacement action reconnect",
        "replacementPlayer",
      ...
      seedScenario(
        "replacement-stale-action-after-resolve",
        "Stale replacement action after resolve",
        "replacementPlayer",
      ),
      seedScenario(
        "replacement-stale-private-post-after-resolve",
        "Stale replacement private post after resolve",
        "replacementPlayer",
      ),
      seedScenario(
        "replacement-stale-private-post-reconnect",
        "Replacement private post reconnect",
        "replacementPlayer",
      ),
      seedScenario(
        "replacement-stale-private-post-after-complete",
        "Stale replacement private post after completion",
        "replacementPlayer",
      ),
      seedScenario(
        "replacement-stale-private-post-after-complete-reload",
        "Completed replacement private channel reload",
        "replacementPlayer",
      ),
      seedScenario("concurrent-host-resolve-race", "Concurrent host resolve race", "host"),
      seedScenario("concurrent-host-resolve-race-reload", "Concurrent host resolve reload", "host"),
      seedScenario("stale-host-resolve-reload", "Stale host resolve reload", "host"),
      seedScenario(
        "stale-host-resolve-reconnect-recovery",
        "Stale host resolve reconnect",
        "host",
      ),
      seedScenario("concurrent-host-advance-race", "Concurrent host advance race", "host"),
      seedScenario("concurrent-host-advance-race-reload", "Concurrent host advance reload", "host"),
      seedScenario("stale-host-advance-reload", "Stale host advance reload", "host"),
      ...
      seedScenario(
        "concurrent-host-deadline-advance-race-reload",
        "Concurrent host deadline advance reload",
        "host",
      ),
      seedScenario("concurrent-host-lifecycle-race", "Concurrent host lifecycle race", "host"),
      seedScenario(
        "concurrent-host-lifecycle-race-reload",
        "Concurrent host lifecycle reload",
        "host",
      ),
      seedScenario("concurrent-host-complete-race", "Concurrent host complete race", "host"),
      seedScenario(
        "concurrent-host-complete-race-reload",
        "Concurrent host complete reload",
        "host",
      ),
      seedScenario("stale-host-prompt-reload", "Stale host prompt reload", "host"),
      seedScenario("stale-host-complete-reload", "Stale host completion reload", "host"),
      seedScenario(
        "stale-host-complete-reconnect-recovery",
        "Stale host completion reconnect",
        "host",
      ),
      seedScenario(
        "concurrent-player-complete-race",
      ...
      seedScenario(
        "public-player-complete-reload",
        "Public player completion reload",
        "player",
      ),
      seedScenario(
        "stale-player-complete-reload",
        "Stale public player completion reload",
        "player",
      ),
      seedScenario("concurrent-host-mixed-advance-race", "Concurrent host mixed advance race", "host"),
      seedScenario(
        "concurrent-host-mixed-advance-race-reload",
        "Concurrent host mixed advance reload",
        "host",
      ),
      ...
      seedScenario("stale-dead-action-conflict", "Stale dead action conflict", "actionPlayer"),
      ...
      seedScenario("replacement-stale-session-after-refresh", "Stale replacement session after refresh", "replacementPlayer"),
      seedScenario("replacement-reconnect-recovery", "Replacement reconnect recovery", "replacementPlayer"),
      seedScenario("replacement-stale-conflict-message", "Replacement stale conflict message", "host"),
      ...
      seedScenario("stale-host-invite-recovery", "Stale host invite recovery", "host"),
      ...
      seedScenario("replacement-stale-action", "Replacement stale action recovery", "player"),
      seedScenario("replacement-stale-private-channel", "Replacement stale private channel", "player"),
      seedScenario("replacement-stale-private-receipts", "Replacement stale private receipts", "player"),
...
function hostStaleControlCheckRows() {
  return [
    ["host-stale-control-milestone", "16/16 covered"],
