...
test("admin route data exposes local hardening proof as a native audit row", async () => {
  ...
  assert.equal(hardening.status, "72 hardening lanes passed");
...
test("admin local hardening detail data carries lane rows", async () => {
  ...
  assert.equal(data.audit.checks.length, 72);
  assert.deepEqual(
    data.audit.checks.map((check) => [check.id, check.status]),
    [
      ...
      ["concurrent-action-race", "passed"],
      ["concurrent-action-race-reload", "passed"],
      ["reconnect-recovery", "passed"],
      ...
      ["stale-same-action-recovery", "passed"],
      ...
      ["concurrent-host-resolve-race-reload", "passed"],
      ...
      ["stale-host-resolve-reload", "passed"],
...
test("admin route data exposes local seed fixture summary as a native audit row", async () => {
  ...
  assert.equal(seed.status, "77 demo scenarios available locally");
...
test("admin local seed fixture detail data carries scenario rows", async () => {
  ...
  assert.equal(data.audit.scenarios.length, 77);
  assert.deepEqual(
