...
preserving the original failure class. A non-mafiascum generated replay lane now covers six
...
thread row from the envelope. Projection rebuilds reproduce that row from the log, and the phase
cursor remains owned only by `GameStarted` / `PhaseAdvanced`.
