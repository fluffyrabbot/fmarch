...
Resolver entrypoints no longer repeat conversion, visibility-family,
Ninja-visibility, win-family, or ITA vote-conflict validation after receiving a
`ValidatedPack`. Action-owned chosen-retaliation policy lookup and trigger-owned
generated-kill/fixpoint lookup sit beside the behavior they parameterize. The
source-boundary contract prevents either redundant admission or those
cross-family accessors from returning to the coordinator. Validation diagnostics
remain the sole rejection surface; resolver behavior for every accepted pack is
unchanged.
