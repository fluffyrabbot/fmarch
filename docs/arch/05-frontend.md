...
| `npm run test:frontend-iab-fixture-bundle` | Generates `target/frontend-in-app-browser-bundle/fixture-replay-bundle.tar` plus `bundle-manifest.json` as a deterministic ustar payload for moving the generated fixture to a Chromium-capable environment. The archive includes `interaction-page.html`, `interaction-page-manifest.json`, replay handoff JSON/Markdown, latest file-backed `browser-run.json`, latest localhost-served `browser-run.json`, imported-run status, and any file or localhost `browser-run-*.png` screenshots that exist, with stable ordering, zero mtimes, fixed mode, and SHA-256 hashes. | Browser behavior, fixture freshness after export, Svelte hydration, command side effects, TCP/network transport, WebSocket delivery, dev-server routing, or full localhost app acceptance. |
| `npm run test:frontend-iab-fixture-bundle-import` | Validates a returned `fixture-replay-bundle.tar`, defaulting to the current bundle or an alternate path via `--source`/`FMARCH_IAB_FIXTURE_BUNDLE_IMPORT`. It parses the deterministic tar, verifies the required fixture/replay/import payload, extracts it under `target/frontend-in-app-browser-bundle-import/extracted`, restores returned localhost fixture artifacts to `target/frontend-in-app-browser-localhost/`, and runs the imported-run contract against the extracted file-backed `browser-run.json`, writing `target/frontend-in-app-browser-bundle-import/bundle-import.json`. | Browser execution, Svelte hydration, command side effects, TCP/network transport, WebSocket delivery, or localhost-backed app acceptance by itself; it promotes imported file evidence only when the extracted imported-run proof is `imported-passed`, while the browser-acceptance boundary separately evaluates the restored localhost fixture artifact. |
| `npm run test:frontend-iab-operator-runbook` | Generates `target/frontend-in-app-browser-operator-runbook/runbook.json` and `.md` from the current bundle, bundle-import, replay handoff, imported-run, completion-audit, and readiness artifacts. It records the four-step external Chromium workflow, exact commands, expected returned files for both file-backed and localhost-served fixture runs, completion-audit/readiness refresh commands, promotion checks, current blocked status, and the remaining localhost acceptance boundary. | Browser execution, fixture replay, imported browser proof, Svelte hydration, command side effects, TCP/network transport, WebSocket delivery, or localhost-backed app acceptance by itself. |
| `npm run test:frontend-iab-replay-help` | Generates `target/frontend-in-app-browser-replay-help/replay-help.json`, `.md`, and `.sh` from the current fixture manifest, replay handoff, bundle manifest, and operator runbook. The helper condenses the external replay into freshen/replay/import/completion-audit commands, expected returned files, route-error promotion requirements, completion-audit refresh, current status, and promotion checks for proving both the file-backed and localhost-served fixture browser-run lanes outside the sandbox. | Browser execution, fixture replay, imported browser proof, Svelte hydration, command side effects, TCP/network transport, WebSocket delivery, or localhost-backed app acceptance by itself. |
| `npm run test:frontend-browser-acceptance-boundary` | Generated browser-acceptance boundary at `target/frontend-browser-acceptance-boundary/browser-acceptance-boundary.json`, derived from localhost role smoke, no-bind Chromium render, no-bind interaction, no-bind keyboard, the prepared file-backed in-app fixture, the direct file-backed fixture browser-run artifact, the localhost-served fixture browser-run artifact, and the imported external file-backed browser-run artifact. It records which lanes are proven, blocked, or fixture-prepared, requires localhost role-smoke promotion to include admin session-grant/recovery-gate form evidence, player main-thread `SubmitPost` ACK refresh evidence, player role-PM `SubmitPost.channel_id="role-pm"` ACK evidence, player tablet-media browser request evidence without original/full/desktop URLs, and moderator `SetSlotStatus`/slot-lifecycle ACK evidence, and requires the direct/imported fixture lanes to carry the player private-channel 403 route-error click/focus evidence before promotion. It keeps fixture browser runs distinct from full localhost app acceptance. | Browser behavior by itself; it is a truth surface over existing artifacts and does not promote prepared fixtures, bind blocks, Chromium launch blocks, or source-blocked imported runs to acceptance. |
...
| `npm run test:frontend-completion-audit` | Generated requirement audit at `target/frontend-completion-audit/completion-audit.json`, derived from the static, SSR, dispatch-bridge, DOM, render, role-smoke, tablet interaction, prepared file-backed in-app browser fixture, direct fixture browser-run artifact, imported external fixture browser-run artifact, and browser-acceptance boundary artifacts. It records shared shell, single root-owned shell architecture, tablet-native interaction posture, player, moderator, admin, route-state, prepared fixture, fixture browser-run/import status, and browser-acceptance status without hand-maintained promotion, including player thread media proof that tablet/small/thumb variants render while original-only images are withheld and player thread pager proof for pending/ACK/reject older-page states. | New behavior by itself; it summarizes existing proof and fails closed when required artifacts are stale or missing, and an imported file-backed browser run still remains distinct from localhost app acceptance. |
| `npm run test:frontend-readiness-summary` | Operator-readable readiness summary at `target/frontend-readiness-summary/readiness-summary.json` and `.md`, derived from the generated proof artifacts. It separates single-root-shell proof, role model/SSR/DOM readiness, no-bind Chromium, localhost browser acceptance, the prepared in-app file fixture, the direct file fixture browser-run artifact, the localhost-served fixture browser-run artifact, and the imported external fixture browser-run artifact, including blocked reasons, feedback-rail status IDs, checked promotion criteria, and per-role promotion failures for missing Chromium/browser evidence such as admin session-grant/recovery-gate form evidence, player `SubmitPost` ACK, or all 10 moderator critical host confirmation clicks. | New frontend behavior, browser proof, or completion by itself; it is a truth surface over current artifacts and does not promote the prepared file fixture, blocked fixture browser-runs, or a blocked imported run to browser acceptance. |
...
The generated completion audit currently reports `status: incomplete` and
`overall.state: not_complete`. It is intentionally not a release claim; it is the live truth
table for the frontend objective.

| Requirement | Current status | Proven by | Still missing |
|---|---|---|---|
| Shared app shell | `dom_proven_browser_blocked` | Static shell/nav/session contract, shared surface-header contract for board/admin/admin-audit/player/moderator first viewports, static first-viewport layout contract for 4/2/1 scan-strip columns and text-wrap guardrails, SSR route shell, DOM surfaces for board/admin/player/moderator. | Chromium focus traversal, visible focus rings, pointer behavior, overlap proof. |
| Tablet-native interaction posture | `source_css_ssr_proven_browser_blocked` | Source scan proves tap-first preload posture and no hover-only selectors/handlers; CSS proof covers shared edge-to-edge safe-area shell, sticky safe-area role/session topbar, controlled overscroll, 44px touch/focus/wrapping/scan-strip guardrails, a safe-area-aware sticky admin operator rail with setup/recovery primary actions before reserved status floors, a safe-area-aware sticky player command rail with primary controls before live receipts, and a safe-area-aware sticky moderator host control rail offset below that shared topbar with primary actions before reserved status floors and before route-level status readouts; SSR proof keeps admin setup/recovery, player vote/post, and moderator critical actions inside explicit thumb-zone containers. | Browser physical thumb reach, pointer delivery, pixel overlap, visible focus rings, and real Tab traversal. |
...
| Player surface | `dom_and_model_proven_browser_blocked` | Player route model, command reject path, SSR player surface with command receipt strip, tablet-safe thread media variant rendering, older-thread pager pending/ACK/reject model and fixture lifecycle evidence, private-review URL, private-channel URL, DOM player/private-review/private-channel surfaces. | Hydrated click/focus retention, nonblank screenshots, pointer behavior, actual image fetch behavior. |
| Moderator/host surface | `dom_and_model_proven_browser_blocked` | Host route/control models, moderator ACK and host-prompt projection paths, SSR host console and command activity rail, DOM moderator surface. | Tablet Chromium focus trap, touch geometry, overlap, hydrated command dispatch. |
| Admin/operator surface | `dom_and_model_proven_browser_blocked` | Admin setup/audit/recovery models, admin reject path, SSR admin and audit detail, DOM admin/audit surfaces. | Hydrated confirm focus traversal and audit click-through screenshots. |
| Route states | `ssr_and_dom_proven` | SSR and DOM empty/loading/reject pages for admin/player/player-private-channel/moderator. | Browser pixel layout and focus traversal for route-state pages. |
| Browser acceptance | `blocked_by_localhost_and_chromium_sandbox` | Structured fallback artifacts record the blocked boundary. | localhost bind and Chromium launch in this sandbox. |
...
- **Media variant fit** — thread images render only projection-provided tablet/small/thumb
  ...
  unavailable row, and model/SSR/DOM proof keeps originals and desktop-only full variants out
  of rendered player thread markup. Browser network proof is still required to prove actual
