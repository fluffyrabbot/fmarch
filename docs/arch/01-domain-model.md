...
A **principal** is the private account authority; a **profile** is its optional
public community presentation. A **game persona** is a game-scoped identity
whose private ownership resolves to a principal. “User” below means the human
behind that authority, not a public identifier to persist in game authorship.
...
> Persisting only the current account would lose which occupancy authored a
> historical post. Slot and occupancy facts preserve that distinction across
> replacement and account erasure.
...
from setup through active play to completion (`game_index.status` is
...
- **Occupant history** — immutable persona/slot occupancy start and end facts,
  with the principal relationship held privately. Replacement ends one occupancy
  and starts another; it does not replace the slot.
- **Lifecycle state** — living, dead, or modkilled. Spectator grants are separate
  from player slots, and replacement is an occupancy transition.
...
### Phase
An ordered, typed segment: `Day`, `Night`, or `Twilight`. `PhaseId` accepts
canonical ids such as `D01`, `N12`, and `D03R2` (a revote). Setup and completion
are game lifecycle states, not extra phase kinds.
A phase has:
- a **deadline** (optional, mutable by the host),
- **posting rules** — who may post (e.g. Night can lock the main thread; dead slots cannot post there),
...
the same channel and author slot across replacement; only the private persona/occupancy authority
changes. Post-start slot/role assignment emits the same declaration when it first
...
### Post
Game player authorship records a **slot and occupancy/persona history**;
community authorship uses the separate profile boundary. Host notices have
explicit host provenance. Public post identity never exposes an account principal.
