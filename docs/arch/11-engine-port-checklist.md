...
5. Determinism fuzzing with random but seeded scenario generation.
   ...
   seeds, runs each of the 32 cases through success and bad-expectation reductions, asserts success
   ...
   `target/operator-proof/current-generated-shrink-matrix-report.tmp.json` with `ok: true`, 16
   families, 32 cases, and the proof boundary that it is bounded local-Postgres coverage rather
   ...
   BackupInheritance, ConversionDeprogramming, ConversionProjectionState, MarkClearVisibility, MarkClearExpiry, PoisonCure, Ignite, ExtraAction, ItemGrant, PrivateNotification, Lovers, and Bomb against local Postgres, writes a versioned report with two cases per family
   ...
   `family_count: 16`, `case_count: 32`, `expected_family_count: 16`, `expected_case_count: 32`,
   ...
   `ok: true`, 16 families, 32 cases, two representative seeds per PGO/Babysitter/Hider/Hunter/BackupInheritance/ConversionDeprogramming/ConversionProjectionState/MarkClearVisibility/MarkClearExpiry/PoisonCure/Ignite/ExtraAction/ItemGrant/PrivateNotification/Lovers/Bomb
...
Continue Phase 4 by extending the generated shrink matrix to backup inheritance projection-state
breadth. Build a multi-phase fixture where targeted backup source designation survives rebuild,
the source dies, backup inherits the source role/alignment in folded `slot_state`, and a
bad-expectation variant mutates the inherited role/alignment or source action. Prove the minimizer
preserves `semantic_expectation`, then raise the matrix manifest from 16/32 to 17/34.
