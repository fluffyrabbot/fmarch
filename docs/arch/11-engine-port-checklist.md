...
5. Determinism fuzzing with random but seeded scenario generation.
   ...
   generated seeds plus two deterministic Hunter, HunterProjectionState, VengefulFixpoint, StrongmanVengefulFixpoint, BodyguardStrongmanVengefulFixpoint, PgoProjectionState, HiderProjectionState, BabysitterProjectionState, LoversProjectionState, BackupInheritance, BackupProjectionState, ConversionDeprogramming,
   ...
   seeds, runs each of the 52 cases through success and bad-expectation reductions, asserts success
   ...
   `target/operator-proof/current-generated-shrink-matrix-report.tmp.json` with `ok: true`, 26
   families, 52 cases, and the proof boundary that it is bounded local-Postgres coverage rather
   ...
   `family_count: 26`, `case_count: 52`, `expected_family_count: 26`, `expected_case_count: 52`,
   ...
   `ok: true`, 26 families, 52 cases, two representative seeds per 
...
Continue Phase 4 by extending generated trigger breadth to Vengeful projection-state fixpoint.
Build a deterministic fixture where a killed Vengeful player generates a retaliation kill, folds
both the source and retaliated target deaths into `slot_state`, and uses a bad-expectation variant
that mutates the retaliated target state or generated-action trace. Prove the minimizer preserves
`semantic_expectation`, then raise the matrix manifest from 26/52 to 27/54.
