...
5. Determinism fuzzing with random but seeded scenario generation.
   ...
   MarkClearVisibility, MarkClearExpiry, PoisonCure, Ignite, ExtraAction, ItemGrant, PrivateNotification, Lovers, and Bomb persistent/generated-action
   seeds, runs each of the 30 cases through success and bad-expectation reductions, asserts success
   ...
   `target/operator-proof/current-generated-shrink-matrix-report.tmp.json` with `ok: true`, 15
   families, 30 cases, and the proof boundary that it is bounded local-Postgres coverage rather
   ...
   BackupInheritance, ConversionDeprogramming, MarkClearVisibility, MarkClearExpiry, PoisonCure, Ignite, ExtraAction, ItemGrant, PrivateNotification, Lovers, and Bomb against local Postgres, writes a versioned report with two cases per family
   ...
   `family_count: 15`, `case_count: 30`, `expected_family_count: 15`, `expected_case_count: 30`,
   ...
   `ok: true`, 15 families, 30 cases, two representative seeds per PGO/Babysitter/Hider/Hunter/BackupInheritance/ConversionDeprogramming/MarkClearVisibility/MarkClearExpiry/PoisonCure/Ignite/ExtraAction/ItemGrant/PrivateNotification/Lovers/Bomb
...
Continue Phase 4 by extending the generated shrink matrix to conversion/deprogramming projection-state breadth.
Build a multi-phase fixture where conversion assigns a temporary role/alignment and records original
role provenance, deprogramming restores that original state, and folded slot/projection state after
rebuild matches the restored role. Add a bad-expectation variant that mutates the original-role
source or restored alignment, prove the minimizer preserves `semantic_expectation`, and only then
raise the matrix manifest from 15/30 to 16/32.
