...
5. Determinism fuzzing with random but seeded scenario generation.
   ...
   generated seeds plus two deterministic Hunter, VengefulFixpoint, BackupInheritance, BackupProjectionState, ConversionDeprogramming,
   ...
   seeds, runs each of the 36 cases through success and bad-expectation reductions, asserts success
   ...
   `target/operator-proof/current-generated-shrink-matrix-report.tmp.json` with `ok: true`, 18
   families, 36 cases, and the proof boundary that it is bounded local-Postgres coverage rather
   ...
   VengefulFixpoint, BackupInheritance, BackupProjectionState, ConversionDeprogramming, ConversionProjectionState, MarkClearVisibility, MarkClearExpiry, PoisonCure, Ignite, ExtraAction, ItemGrant, PrivateNotification, Lovers, and Bomb against local Postgres, writes a versioned report with two cases per family
   ...
   `family_count: 18`, `case_count: 36`, `expected_family_count: 18`, `expected_case_count: 36`,
   ...
   `ok: true`, 18 families, 36 cases, two representative seeds per PGO/Babysitter/Hider/Hunter/VengefulFixpoint/BackupInheritance/BackupProjectionState/ConversionDeprogramming/ConversionProjectionState/MarkClearVisibility/MarkClearExpiry/PoisonCure/Ignite/ExtraAction/ItemGrant/PrivateNotification/Lovers/Bomb
...
Continue Phase 4 by extending generated trigger fixpoint breadth to strongman vengeful retaliation.
Build a deterministic fixture where an `unstoppable_vengeful` death emits exactly one Strongman
follow-up retaliation action, proves protection/bodyguard bypass in the generated-kill trace, and
uses a bad-expectation variant that mutates the generated action id or bypass decision. Prove the
minimizer preserves `semantic_expectation`, then raise the matrix manifest from 18/36 to 19/38.
