...
5. Determinism fuzzing with random but seeded scenario generation.
   ...
   generated seeds plus two deterministic Hunter, BackupInheritance, BackupProjectionState, ConversionDeprogramming,
   ...
   seeds, runs each of the 34 cases through success and bad-expectation reductions, asserts success
   ...
   `target/operator-proof/current-generated-shrink-matrix-report.tmp.json` with `ok: true`, 17
   families, 34 cases, and the proof boundary that it is bounded local-Postgres coverage rather
   ...
   BackupInheritance, BackupProjectionState, ConversionDeprogramming, ConversionProjectionState, MarkClearVisibility, MarkClearExpiry, PoisonCure, Ignite, ExtraAction, ItemGrant, PrivateNotification, Lovers, and Bomb against local Postgres, writes a versioned report with two cases per family
   ...
   `family_count: 17`, `case_count: 34`, `expected_family_count: 17`, `expected_case_count: 34`,
   ...
   `ok: true`, 17 families, 34 cases, two representative seeds per PGO/Babysitter/Hider/Hunter/BackupInheritance/BackupProjectionState/ConversionDeprogramming/ConversionProjectionState/MarkClearVisibility/MarkClearExpiry/PoisonCure/Ignite/ExtraAction/ItemGrant/PrivateNotification/Lovers/Bomb
...
Continue Phase 4 by extending generated trigger fixpoint breadth to vengeful-style retaliation.
Build a deterministic fixture where a killed slot emits exactly one follow-up retaliation action,
the generated action resolves through the trigger fixpoint without duplicating on rebuild, and a
bad-expectation variant mutates the generated action id or retaliation cause. Prove the minimizer
preserves `semantic_expectation`, then raise the matrix manifest from 17/34 to 18/36.
