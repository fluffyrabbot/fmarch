...
2. Host/admin trace inspection.
   ...
   still names an async test in the ordinary pipeline or dedicated semantic-audit target; the same test validates
...
5. Determinism fuzzing with random but seeded scenario generation.
   ...
   multi-node, production, browser, or exhaustive state-space behavior.` This command was run
	   ...
	   carry exact trusted metadata for `projection_table_count: 32` and `resolution_phase_count: 1`.
   ...
   fmarch proof surfaces and two explicit out-of-scope/non-resolution buckets. The unported
	   ...
	   shrink gap-audit, and command/projection proof rows, including `projection_table_count: 32`
