...
5. Determinism fuzzing with random but seeded scenario generation.
   ...
   generated seeds plus two deterministic Hunter, VengefulFixpoint, StrongmanVengefulFixpoint, BodyguardStrongmanVengefulFixpoint, PgoProjectionState, HiderProjectionState, BabysitterProjectionState, LoversProjectionState, BackupInheritance, BackupProjectionState, ConversionDeprogramming,
   ...
   seeds, runs each of the 50 cases through success and bad-expectation reductions, asserts success
   ...
   `target/operator-proof/current-generated-shrink-matrix-report.tmp.json` with `ok: true`, 25
   families, 50 cases, and the proof boundary that it is bounded local-Postgres coverage rather
   ...
   `family_count: 25`, `case_count: 50`, `expected_family_count: 25`, `expected_case_count: 50`,
   ...
   `ok: true`, 25 families, 50 cases, two representative seeds per 
...
Continue Phase 4 by extending generated trigger breadth to Hunter projection-state retaliation.
Build a deterministic fixture where a killed Hunter retaliates against a chosen target, folds both
the Hunter death and retaliation death into `slot_state`, and uses a bad-expectation variant that
mutates the retaliated target state or retaliation cause. Prove the minimizer preserves
`semantic_expectation`, then raise the matrix manifest from 25/50 to 26/52.
