...
6. Add a second Railway Bucket named `subject-authority`. It is a shared authority for both API replicas, not a mounted volume, and must never be the media bucket or be cloned across staging and production. Bind its five S3 reference variables plus an independently generated authority UUID, wrapping key/KID, journal-authentication key/KID, and revision from `deploy/railway/api.env.example`. Before the first normal API start, run the exact release image once with `fmarch-server --bootstrap-subject-authority`; this create-only command writes and verifies the immutable manifest and refuses an existing authority. Normal startup never creates a manifest: it binds an empty database to that genesis, lists and reconciles revocations, and verifies every active subject key before listeners start. Copy the remaining template values into Railway Variables. Create a WorkOS AuthKit environment and configure its sign-in endpoint as `https://<frontend>/auth/sign-in`, redirect URI as `https://<frontend>/auth/callback`, and default sign-out redirect/application homepage as the exact canonical root `https://<frontend>/`. The application deliberately sends no caller-controlled `return_to`; do not configure a wildcard or alternate sign-out target. Fill in the WorkOS client id, issuer, and JWKS URL. The template is explicitly WorkOS-only (`FMARCH_CLASSIC_AUTH=0`), but sign-in selection does not own identity delivery. Configure `FMARCH_IDENTITY_DELIVERY_ENDPOINT`, `FMARCH_IDENTITY_DELIVERY_PROVIDER_ID`, and an environment-isolated `FMARCH_IDENTITY_DELIVERY_AUTH_TOKEN` for a real HTTPS provider in both WorkOS-only and classic-plus-WorkOS deployments. The provider is either a relay speaking the provider-neutral `http-json` protocol or Resend directly (`FMARCH_IDENTITY_DELIVERY_ADAPTER=resend`, endpoint `https://api.resend.com/emails`, a sending-only API key as the token, a sender on a Resend-verified domain in `FMARCH_IDENTITY_DELIVERY_SENDER`, and the canonical frontend origin in `FMARCH_IDENTITY_DELIVERY_LINK_ORIGIN`). Process startup and hosted release preflight both fail closed when the delivery transport is absent; preflight also rejects invalid, placeholder-shaped, or cross-environment shared authority. For a fresh database, set `FMARCH_BOOTSTRAP_ADMIN_WORKOS_USER_ID` to the immutable WorkOS user id that should receive the first GlobalAdmin grant; an optional label is display-only. Startup grants it only when no active GlobalAdmin exists. Remove the bootstrap variables after the first successful boot.
   Treat `FMARCH_IDENTITY_DELIVERY_PROVIDER_ID` as an immutable generation, not an adapter label. Use an environment-specific, versioned value such as `staging-mail-v1`; production must use a distinct value such as `production-mail-v1`, and hosted preflight rejects the generic `http-json`. The durable nonsecret fingerprint covers the adapter protocol and exact endpoint but deliberately excludes the bearer token. Rotate only the token in place when the provider accepts old and new credentials through full replica drain; if overlap is impossible, use a fresh generation. Any endpoint or protocol change also requires a fresh, never-before-used generation id, and cutover remains blocked until queued work and live attempts for the previous generation have drained. Retired generation ids remain reserved permanently. The provider must implement the delivery-v2 contract: validate the environment-specific generation; interpret `lease_expires_at` as Unix epoch seconds; reject an elapsed deadline; recheck it immediately before the credential side effect; and guarantee that no such effect commits at or after it. The configured margin must satisfy `margin >= ceil(max(database clock - provider clock) + max(provider deadline-to-no-effect quiescence lag))`; the second term is zero only when the provider's final deadline check and effect are atomic. The provider must reject a declared margin below that verified bound and deduplicate the stable delivery id. It may report definitive completion only with a strict `fmarch.identity-delivery-result.v2` body that echoes the exact provider generation, delivery id, and attempt token and uses a valid status-specific shape. Every non-success HTTP status and malformed, unknown, or mismatched body is uncertain remote execution. Set `FMARCH_IDENTITY_DELIVERY_PROVIDER_CLOCK_SKEW_MARGIN_MS` at or above the verified cross-clock bound; startup and hosted preflight require a whole-second margin and a claim lease that exceeds provider I/O plus three database phases plus the margin plus a one-second database-clock quantization reserve. The later anonymous generation-fence expiry is database-internal and equals the wire deadline plus that margin. The API retains uncertain attempt fences until that later expiry, and cutover stays blocked even if privacy erasure has removed the delivery row. Required runtime workers become ready before startup commits an irreversible generation change.
   A provider-unavailable observation suspends that exact generation across every replica. Background delivery and explicit retry remain fenced while suspended; a retry is never a health probe. A GlobalAdmin may restore service only through the credential-free, nonce-echoed provider probe. Failed or stale probes preserve the suspension.
