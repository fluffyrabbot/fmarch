...
| Admin | Admin focus traversal and DOM smoke passed, including the audit-detail surface. Contracts cover the roving inbox model, selected canvas, cohost and create-game confirmations, recovery-gate confirmation, cancellation, and task-key navigation. | Actual roving-focus usability, announcement of selection and authority, confirmation comprehension, and focus return after cancel in the hydrated application. |
...
| Admin | Traverse the operator inbox with roving task focus, open the selected decision canvas, review cohost/create-game confirmation, and return focus on cancel | not run | not run | not run | |
