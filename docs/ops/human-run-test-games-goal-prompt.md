...
Current state: the frontend browser acceptance lane is proven by `npm run test:frontend-role-proof:browser`, and `npm run dev:test-game` starts/uses a Rust API plus SvelteKit frontend, seeds one mafiascum D01 game through `/commands`, creates opaque browser-login tokens, prints role login URLs, and writes `target/dev-test-game/session.{json,md}`. The harness now supports friendly named games, default reuse, explicit `--reset` and `--reuse`, port and Postgres preflights, and optional browser-entry verification. Treat this as a developer harness, not a release/beta claim.
...
2. `npm run test:dev-test-game-contract` proves argument parsing, session-card shape, seed command plan shape, and URL/token output without starting servers.
3. `npm run test:dev-test-game-live` runs the harness with `--reset --verify --no-keepalive`, validates the emitted session artifact, and verifies host/player browser entry through the real API/SvelteKit boundary.
