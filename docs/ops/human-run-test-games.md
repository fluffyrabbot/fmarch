...
controls, and unlocks D02 again for the remaining seeded flows, keeps a full
host page frozen on the D01 deadline control until its stale `ExtendDeadline`
click renders a host command activity `Reject PhaseLocked` receipt, refreshes to
...
verifying the eighteen hardening lane rows above.
...
stale deadline advance recovery, stale host resolve recovery, stale host deadline recovery, stale cohost deadline recovery,
