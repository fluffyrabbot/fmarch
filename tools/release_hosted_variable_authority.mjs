...
export function validateHostedVariables({
  ...
  for (const [environment, apiVariables, migratorVariables, environmentId] of [
    ["staging", stagingApi, stagingMigrator, DEFAULTS.stagingEnvironmentId],
    ["production", productionApi, productionMigrator, DEFAULTS.productionEnvironmentId],
  ]) {
    for (const [process, variables] of [
      ["API", apiVariables],
      ["migrator", migratorVariables],
    ]) {
      assert.equal(
