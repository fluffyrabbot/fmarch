...
import {
  ...
  normalDayVotePrivacySurfaceFixture as sharedNormalDayVotePrivacySurfaceFixture,
  normalNightActionResolutionPrivacySurfaceFixture as sharedNormalNightActionResolutionPrivacySurfaceFixture,
  normalPostDayVoteAdvanceSurfaceFixture as sharedNormalPostDayVoteAdvanceSurfaceFixture,
  normalResolutionPrivacySurfaceFixture as sharedNormalResolutionPrivacySurfaceFixture,
  playerActionSubmissionRoleSurfaceFixture as sharedPlayerActionSubmissionRoleSurfaceFixture,
  targetDayVoteReceiptSurfaceFixture as sharedTargetDayVoteReceiptSurfaceFixture,
  targetPostDayVoteAdvanceSurfaceFixture as sharedTargetPostDayVoteAdvanceSurfaceFixture,
...
test("dev test-game spine orchestrators expose stable proof order and env maps", async () => {
  ...
  assert.equal(
    ...
    `node ${devTestGameHostedIdentityProgressionAdminProofBatchScript}`,
  );
  assert.equal(
    devTestGameHostedIdentityProgressionSummaryCommand,
...
test("dev test-game next-action derives one local recovery command from the manifest", () => {
  ...
  assert.deepEqual(
    devTestGameAdminSpinePlan
      .slice(
        hostedIdentityAdminSpinePhaseStart,
        hostedIdentityAdminSpinePhaseStart + 4,
      )
      .map((step) => ({
        script: step.script,
        sequenceStage:
  ...
  const missingProofGraphNextActionHandoffAction = buildDevTestGameNextAction(
    freshManifest,
    {
      generatedAt: "2026-06-26T00:00:01.000Z",
      opsArtifacts: devTestGameOpsArtifactsFixture(),
      raceCoverage: devTestGameRaceCoverageFixture(),
      releaseReadinessChecklist: devTestGameReleaseReadinessChecklistFixture({
        includeProofGraphNextActionHandoffCheck: false,
        unproven: [
          {
            id: "hosted-concurrent-race-matrix",
            status: "unproven",
  ...
  assertDevTestGameNextAction(missingProofGraphNextActionHandoffAction);
  assert.deepEqual(missingProofGraphNextActionHandoffAction.nextAction, {
    command: "npm run test:dev-test-game-proof-graph-admin-proof",
    reason: "release-readiness-local-check-missing",
    status: "blocked",
    localCheck: {
  ...
  assert.deepEqual(
    missingProofGraphNextActionHandoffAction.localReadinessDependencyTrace,
    {
      strategy: "local-readiness-dependency-before-hosted-work",
      candidateCount: 1,
      selectedCheckId: "local-proof-graph-next-action-handoff",
      candidates: [
        {
          rank: 1,
          id: "local-proof-graph-next-action-handoff",
  ...
  const missingProofGraphTerminalValidationAction = buildDevTestGameNextAction(
    freshManifest,
    {
      generatedAt: "2026-06-26T00:00:01.000Z",
      opsArtifacts: devTestGameOpsArtifactsFixture(),
      raceCoverage: devTestGameRaceCoverageFixture(),
      releaseReadinessChecklist: devTestGameReleaseReadinessChecklistFixture({
        includeProofGraphTerminalValidationCheck: false,
        unproven: [
          {
            id: "hosted-concurrent-race-matrix",
            status: "unproven",
  ...
  assertDevTestGameNextAction(missingProofGraphTerminalValidationAction);
  assert.deepEqual(missingProofGraphTerminalValidationAction.nextAction, {
    command: "npm run test:dev-test-game-proof-graph-admin-proof",
    reason: "release-readiness-local-check-missing",
    status: "blocked",
    localCheck: {
  ...
  assert.deepEqual(
    missingProofGraphTerminalValidationAction.localReadinessDependencyTrace,
    {
      strategy: "local-readiness-dependency-before-hosted-work",
      candidateCount: 1,
      selectedCheckId: localProofGraphTerminalValidationCheckId,
      candidates: [
        {
          rank: 1,
          id: localProofGraphTerminalValidationCheckId,
  ...
  assert.deepEqual(
    blockedPreflightAction.nextAction.unproven.selectedProductionFeatureGraph,
    {
      ...
      selectedSpineTargetRoleUrl:
        coreLoopSpineTargetsFixture().roleUrlHrefs["d02-n02-host"],
      featureTargetKind:
        resolvedFeatureSpineTargetFixture("host-phase-control").featureTargetKind,
...
test("dev test-game proof graph records local proof role URLs and recovery edges", () => {
  ...
  const proofGraphAdminGeneratedFrom =
    buildProofGraphAdminGeneratedFrom(proofGraphAdminSource);
  const proofGraphAdminRequirements =
    buildProofGraphAdminProofRequirements(proofGraphAdminSource);
  assert.deepEqual(
  ...
  assert.equal(proofGraphAdminRequirements.auditId, localAdminAuditIds.proofGraph);
  assert.deepEqual(
    proofGraphAdminRequirements.requiredChecks,
    [
      ...proofGraphAdminGeneratedFrom.nodeIds,
      ...
      ...proofGraphAdminGeneratedFrom.evidenceObjectRowIds,
      ...proofGraphAdminGeneratedFrom.receiptArtifactRowIds,
      ...proofGraphAdminGeneratedFrom.edgeRowIds,
    ],
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    ...
    graph.summary.diagnosticProofSummary.rows.map((row) => row.id),
  );
  assert.deepEqual(
  ...
  assert(
    proofGraphAdminRequirements.requiredRelatedDestinations.some(
      (destination) =>
  ...
  assert(
    proofGraphAdminRequirements.requiredRelatedDestinations.some(
      (destination) =>
        destination.linkId === "admin-proof:hosted-evidence-lane" &&
        destination.requiredHostedHandoffBlockedChecks.includes(
          "raw-evidence-real-hosted-target",
        ),
    ),
  );
    ...
    localDevelopmentSpine: {
      ...
      checks: [
        ...
        ...(includeProofGraphHandoffCheck
          ...
          : []),
        ...(includeProofGraphHandoffCheck && includeProofGraphNextActionHandoffCheck
          ? [
              {
                id: "local-proof-graph-next-action-handoff",
          ...
          : []),
        ...(includeProofGraphHandoffCheck &&
        includeProofGraphTerminalValidationCheck
          ? [
              {
                id: localProofGraphTerminalValidationCheckId,
...
function coreLoopAdminProofFixture() {
  ...
  const proof = {
    ...
    adminRoleSurface: {
      ...
      visibleSpineCycles: [...coreLoopSpineRows.cycles],
      visibleSpineRoleUrls: [...coreLoopSpineRows.roleUrls],
      visibleSpineCheckpoints: [...coreLoopSpineRows.checkpoints],
    ...
    targetResolutionReceiptSurface:
      sharedTargetResolutionReceiptSurfaceFixture({
        game: coreLoopAdminProofFixtureGameId,
      }),
    normalResolutionPrivacySurface:
      sharedNormalResolutionPrivacySurfaceFixture({
        game: coreLoopAdminProofFixtureGameId,
      }),
    targetDayVoteReceiptSurface: sharedTargetDayVoteReceiptSurfaceFixture({
      game: coreLoopAdminProofFixtureGameId,
    }),
    normalDayVotePrivacySurface: sharedNormalDayVotePrivacySurfaceFixture({
      game: coreLoopAdminProofFixtureGameId,
    }),
    hostPhaseTransitionSurface: sharedHostPhaseTransitionSurfaceFixture({
      game: coreLoopAdminProofFixtureGameId,
    }),
    targetPostDayVoteAdvanceSurface:
      sharedTargetPostDayVoteAdvanceSurfaceFixture({
        game: coreLoopAdminProofFixtureGameId,
      }),
    normalPostDayVoteAdvanceSurface:
      sharedNormalPostDayVoteAdvanceSurfaceFixture({
        game: coreLoopAdminProofFixtureGameId,
      }),
