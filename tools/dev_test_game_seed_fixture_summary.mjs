...
function demoScenarios({ roles, laneIds }) {
  ...
  return [
    ...
    scenario({
      id: "concurrent-replacement-private-post-race-reload",
      title: "Concurrent replacement private-post reload",
      role: "player",
      provenBy: ["concurrent-replacement-private-post-race-reload"].filter(hasLane),
      note: "After the replacement private-post race settles, Mira's stale private-channel role URL reloads to scoped-channel denial while host/API projections show Rowan as current Slot 7.",
    }),
    scenario({
    ...
    scenario({
      id: "concurrent-replacement-vote-race-reload",
      title: "Concurrent replacement vote reload",
      role: "player",
      provenBy: ["concurrent-replacement-vote-race-reload"].filter(hasLane),
      note: "After the replacement vote race settles, Mira's stale board role URL reload evidence keeps command-state forbidden while host/API projections show Rowan as current Slot 7.",
    }),
    scenario({
    ...
    scenario({
      id: "concurrent-replacement-action-race-reload",
      title: "Concurrent replacement action reload",
      role: "player",
      provenBy: ["concurrent-replacement-action-race-reload"].filter(hasLane),
      note: "After the replacement action race settles, the stale outgoing role URL cannot retry while Rowan's current Slot 4 role URL reloads to the correct action surface.",
    }),
    scenario({
    ...
    scenario({
      ...
      title: "Concurrent host resolve reload",
      role: "host",
      provenBy: ["concurrent-host-resolve-race-reload"].filter(hasLane),
      note: "After two host role pages race D02 resolve_phase, both host role URLs reload to locked D02 with only unlock/advance phase controls and matching API phase truth.",
    }),
    scenario({
    ...
    scenario({
      ...
      title: "Concurrent host deadline advance reload",
      role: "host",
      provenBy: ["concurrent-host-deadline-advance-race-reload"].filter(
        hasLane,
      ),
      note: "After two host role pages race D01 advance_phase_by_deadline, both host role URLs reload to open N01 with current resolve/lock controls, no carried deadline, and matching API phase truth.",
    }),
    scenario({
    ...
    scenario({
      ...
      title: "Concurrent host lifecycle reload",
      role: "host",
      provenBy: ["concurrent-host-lifecycle-race-reload"].filter(hasLane),
      note: "After two host role pages race Mark dead and Modkill slot, both host role URLs plus the affected player role URL reload to one terminal Slot 7 status with stale lifecycle and player controls absent and API slot truth matching.",
    }),
    scenario({
    ...
    scenario({
      ...
      title: "Concurrent host complete reload",
      role: "host",
      provenBy: ["concurrent-host-complete-race-reload"].filter(hasLane),
      note: "After two host role pages race CompleteGame, both host role URLs reload to revealed completed-game truth with CompleteGame hidden and API slot reveals matching.",
    }),
    scenario({
    ...
    scenario({
      ...
      title: "Stale host completion reconnect",
      role: "host",
      provenBy: ["stale-host-complete-reconnect-recovery"].filter(hasLane),
      note: "After stale host CompleteGame recovery reloads to revealed endgame truth, the same host role URL drops its live projection, reconnects, and recovers completed host state with the CompleteGame action hidden.",
    }),
    scenario({
    ...
    scenario({
      ...
      title: "Concurrent host mixed advance reload",
      role: "host",
      provenBy: ["concurrent-host-mixed-advance-race-reload"].filter(hasLane),
      note: "After host role pages race D01 advance_phase and advance_phase_by_deadline, both host role URLs reload to open N01 with current resolve/lock controls, no stale advance controls, no carried deadline, and matching API phase truth.",
    }),
    scenario({
    ...
    scenario({
      ...
      title: "Stale action reconnect recovery",
      role: "actionPlayer",
      provenBy: ["stale-action-reconnect-recovery"].filter(hasLane),
      note: "After the stale N01 action rejects and refreshes to D02, the same action-player role URL reloads current D02, drops its live projection, reconnects, and recovers current state with no stale factional_kill control.",
    }),
    scenario({
