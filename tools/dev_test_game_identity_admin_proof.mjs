...
export function identityAdminProofCase() {
  return {
    ...
    buildEvidence: ({ source: identityProof, adminRoleSurface }) => ({
      ...
      proofBoundary:
        "Local SvelteKit admin role URL with fixture admin authority over the auth invite-role identity adapter proof. Proves the saved local identity-adapter evidence, including password rotation, hashed single-use account recovery, and bounded two-tier local credential-attempt throttling, is discoverable from the seeded admin overview and inspectable in a native admin audit detail route with role surfaces and lifecycle checks visible; it does not prove hosted accounts, invite delivery, hosted recovery delivery or traffic, distributed or edge abuse controls, hosted audit retention/export, beta readiness, or production readiness.",
      generatedFrom: {
