  ...
  const localChecks = [
    ...
    },
    {
      ...
      laneIds: [
        ...
        ...hostStaleControlLaneIds.slice(3, 8),
...
function assertCoreLoopPostDayThreePlayerSurfaceProof({
  ...
  if (
    ...
    throw new Error("core-loop admin proof missing post-Day 3 player surface");
  }
  if (
    expectedPrivateReceipt &&
    (proof.privateNotice?.id !== "notification-1" ||
  ...
  if (
    ...
    throw new Error("core-loop admin proof leaked post-Day 3 target receipt");
  }
}
...
function assertCoreLoopCompletedPrivateChannelReloadProof({
  ...
  if (
    ...
    throw new Error("core-loop admin proof missing completed private reload shell");
  }
  for (const snapshotCase of completedPrivateChannelReloadSnapshotAssertionCases({
    proof,
    scenario,
  })) {
    assertCoreLoopCompletedPrivateChannelSnapshot(snapshotCase);
  }
}
...
function assertCoreLoopStaleCompletedPrivatePostRecoveryProof({
  ...
  if (
    ...
    throw new Error(
      "core-loop admin proof missing stale completed private post recovery",
    );
  }
...
function assertCoreLoopCompletedPrivateChannelSnapshot({
  snapshot,
  label,
  expectedBoundary,
  rejectedBody = null,
  ...
  if (
    ...
    throw new Error(
      `core-loop admin proof missing ${label} completed private channel closure`,
    );
  }
}
