...
async function submitConcurrentHostDeadlineAdvanceRace({
  ...
  return {
    ...
    apiPhaseAfterRace: hostStateAfterRace.phase,
    proof:
      "Two seeded host role pages submitted D01 advance_phase_by_deadline concurrently with distinct command ids; one ACKed with deadline evidence plus phase advance, one rejected with InvalidTarget stale-deadline recovery, and both browser projections plus the API converged to open N01 with no carried deadline.",
  };
}
...
async function verifyConcurrentPlayerVoteResolveRace({
  ...
  try {
    ...
    return {
      ...
      apiDayVoteOutcomesAfterRace,
      proof:
        "A disposable player role URL and host role URL raced SubmitVote against ResolvePhase through /commands, accepted only vote-before-resolve ACK ordering or PhaseLocked rejection, then refreshed browser and API projections to locked D01 day-vote outcome truth with vote controls removed.",
    };
  } finally {
    await hostEntry.context.close().catch(() => {});
    await playerEntry.context.close().catch(() => {});
...
async function verifyConcurrentPlayerActionAdvanceRace({
  ...
  try {
    ...
    return {
      ...
      apiHostStateAfterRace,
      proof:
        "A disposable action-player role URL froze an enabled N01 factional_kill control, the host role URL resolved N01 and raced AdvancePhase against the stale action click, then both role URLs and API projections converged to open D02 with no stale action appended.",
    };
  } finally {
    await hostEntry.context.close().catch(() => {});
