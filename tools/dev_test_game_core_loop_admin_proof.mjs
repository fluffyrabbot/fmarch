...
const coreLoopRoleSurfaceProofFunctions = {
  hostLifecycleControlCheckpoint: proveHostLifecycleControlCheckpoint,
  playerActionSubmissionCheckpoint: provePlayerActionSubmissionCheckpoint,
  targetResolutionReceiptSurface: proveTargetResolutionReceiptSurface,
  normalResolutionPrivacySurface: proveNormalResolutionPrivacySurface,
  targetDayVoteReceiptSurface: proveTargetDayVoteReceiptSurface,
  normalDayVotePrivacySurface: proveNormalDayVotePrivacySurface,
  hostPhaseTransitionSurface: proveHostPhaseTransitionSurface,
  targetPostDayVoteAdvanceSurface: proveTargetPostDayVoteAdvanceSurface,
  normalPostDayVoteAdvanceSurface: proveNormalPostDayVoteAdvanceSurface,
  nightActionResolutionReceiptSurface:
    proveNightActionResolutionReceiptSurface,
  normalNightActionResolutionPrivacySurface:
    proveNormalNightActionResolutionPrivacySurface,
  hostNightActionTransitionSurface: proveHostNightActionTransitionSurface,
  dayThreeVoteResolutionSurface: proveDayThreeVoteResolutionSurface,
  postDayThreeResolutionSurface: provePostDayThreeResolutionSurface,
  nightThreeEmptyResolutionSurface: proveNightThreeEmptyResolutionSurface,
  dayFourSurvivorRoleSurface: proveDayFourSurvivorRoleSurface,
  nightFourNoActionSurface: proveNightFourNoActionSurface,
  nightFourNoActionResolutionSurface: proveNightFourNoActionResolutionSurface,
  postNightFourTransitionSurface: provePostNightFourTransitionSurface,
  dayFiveNoLynchResolutionSurface: proveDayFiveNoLynchResolutionSurface,
  completedGameEndgameSurface: proveCompletedGameEndgameSurface,
...
async function runCoreLoopRoleSurfaceProofs({
  ...
  for (const proofCase of coreLoopRoleSurfaceProofCases) {
    ...
    if (typeof prove !== "function") {
      throw new Error(
        `core loop role surface proof is missing browser function: ${proofCase.proofKey}`,
      );
    }
    surfaces[proofCase.surfaceKey] = await prove({
      browser,
      frontendBaseUrl,
      ...proofCase.argsFrom({ proofRun, roleProofUrls }),
    });
  }
  return surfaces;
}
...
function hostRaceSurfacesFromProofRun(proofRun) {
...
export function coreLoopAdminProofCase() {
  return {
    ...
    prove: async ({ browser, frontendBaseUrl, source: proofRun }) => {
      ...
      const roleSurfaces = await runCoreLoopRoleSurfaceProofs({
        browser,
        frontendBaseUrl,
        proofRun,
        roleProofUrls,
...
async function proveNightFourNoActionSurface({
  browser,
  frontendBaseUrl,
  hostRoleUrl,
  actionPlayerRoleUrl,
  ...
  const nightFourNoActionProof = await proveNightFourPlayerNoActionSurface({
    browser,
    frontendBaseUrl,
    roleUrl: actionPlayerRoleUrl,
  });
...
async function proveNightFourNoActionResolutionSurface({
  browser,
  frontendBaseUrl,
  hostRoleUrl,
  actionPlayerRoleUrl,
  ...
  const actionPlayerPrivacyProof = await provePostDayThreePlayerSurface({
    ...
    threadBody: "Night 4 resolved with no player action.",
    dayVoteOutcomesRows: [
...
async function proveNightFourPlayerNoActionSurface({
  browser,
  frontendBaseUrl,
  roleUrl,
}) {
  ...
  const commandRequests = [];
  try {
    await installNightFourNoActionBrowserRoutes(page, {
      commandRequests,
    });
    await page.context().addCookies([
      {
    ...
    const checkpoint = await page.evaluate(() => {
      const commandState = window.__fmarchPlayerProjection?.commandState ?? null;
      const privateQueue =
        window.__fmarchPlayerProjection?.privateQueueBoundary ?? null;
      return {
        phaseId: commandState?.phase?.phaseId ?? null,
        phaseState: commandState?.phase?.locked ? "locked" : "open",
        actorSlot: commandState?.actorSlot ?? null,
        actionCount: commandState?.actions?.length ?? null,
        submitActionControls: document.querySelectorAll(
          '[data-testid="player-action-commands"] button[data-action^="submit_action"]',
        ).length,
        voteTargetCount: commandState?.voteTargets?.length ?? null,
        privateCount: privateQueue?.count ?? 0,
      };
    });
    return {
      status: "passed",
...
async function installNightFourNoActionBrowserRoutes(
  page,
  { commandRequests },
) {
  await page.route("**/commands", async (route) => {
    ...
    await fulfillJson(
      route,
      {
        ...
        body: {
          kind: "Reject",
          body: {
            error: "NoLegalNightFourAction",
            retryable: false,
            message:
              "Night 4 no-action surface does not accept player commands",
          },
        },
      },
      409,
  ...
  await page.route("**/games/*/player-command-state?**", async (route) => {
    await fulfillJson(
      route,
      seededNightFourNoActionPlayerCommandState({
        boundary:
  ...
  await page.route("**/games/*/host-console-state?**", async (route) => {
    await fulfillJson(
      ...
      hostPhaseTransitionConsoleState({
        phaseId: "N04",
        locked: resolved,
        seq: resolved ? 916 : 914,
        boundary: resolved
          ? "Seeded browser ResolvePhase ACK locked Night 4 with no player actions."
          : "Seeded browser host opened Night 4 with no player actions.",
      }),
    );
  });
  await page.route("**/games/*/votecount?**", async (route) => {
...
function seededNightFourNoActionPlayerCommandState({
  boundary,
}) {
  return {
    game: "seeded-night-four-no-action-player",
    actorSlot: "slot-7",
    actorAlive: true,
    actorStatus: "alive",
    roleKey: "mafia_goon",
...
function seededStaleNightFourActionPlayerCommandState({ boundary }) {
  return {
    game: "seeded-stale-night-four-action-player",
    actorSlot: "slot-7",
    actorAlive: true,
    actorStatus: "alive",
    roleKey: "mafia_goon",
...
export function assertCoreLoopAdminProof(evidence) {
  ...
  assertCoreLoopHostRaceSurfaces(evidence);
  ...
  assertNightFourNoActionSurface(
    evidence.nightFourNoActionSurface,
  );
  assertNightFourNoActionResolutionSurface(
...
function assertCoreLoopHostRaceSurfaces(evidence) {
  for (const raceCase of hostControlRaceScenarioCases()) {
    assertHostControlRaceSurfaceCase({
      raceCase,
      surface: evidence[raceCase.surfaceField],
      includeEvidenceInError: true,
    });
  }
}
...
function assertNightFourNoActionSurface(nightFourNoActionSurface) {
  const expectedGame = gameFromRoleUrl(
...
function assertNightFourNoActionResolutionSurface(
