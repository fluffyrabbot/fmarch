use std::{collections::BTreeMap, env, fs, future::Future, path::Path};
...
#[derive(Debug, Clone)]
pub struct MinimizeFixtureArtifacts {
    pub report: serde_json::Value,
    pub reduced_fixture: serde_json::Value,
}

pub async fn run_cli<I>(args: I) -> Result<(), Box<dyn std::error::Error>>
where
    I: IntoIterator<Item = String>,
{
    let args = Args::parse(args)?;
    let fixture_json = fs::read_to_string(Path::new(&args.fixture_path))?;
    ...
    let artifacts = minimize_fixture_json(
        ...
        &fixture_json,
        args.reduce,
        args.write_reduced_path.as_deref(),
    )
    .await?;
...
pub async fn minimize_fixture_json(
    pool: &PgPool,
    fixture_json: &str,
    reduce: bool,
    reduced_path_label: Option<&str>,
) -> Result<MinimizeFixtureArtifacts, String> {
    let fixture: NightFixture = serde_json::from_str(fixture_json)
        .map_err(|err| format!("parse minimizer fixture: {err}"))?;
    let original = run_fixture(pool, &fixture).await;
    let reduction_target = ReductionTarget::from_report(&fixture, &original);
    let (fixture, steps, minimized) = if reduce {
    ...
    let write_reduced = reduced_path_label.map(|path| WriteReducedReport::new(path, &reduction));
    let reduced_fixture = serde_json::to_value(&fixture)
    ...
    let report =
        serde_json::to_value(report).map_err(|err| format!("serialize minimizer report: {err}"))?;
    Ok(MinimizeFixtureArtifacts {
        report,
        reduced_fixture,
    })
}
...
async fn minimize_fixture(
    ...
    minimize_fixture_with(fixture, target, |candidate| async move {
        run_fixture(pool, &candidate).await
    })
    .await
}

async fn minimize_fixture_with<F, Fut>(
    mut fixture: NightFixture,
    target: ReductionTarget,
    mut evaluate: F,
) -> (NightFixture, Vec<ReductionStep>, RunReport)
where
    F: FnMut(NightFixture) -> Fut,
    Fut: Future<Output = RunReport>,
{
    ...
    while changed {
        ...
        for index in 0..fixture.votes.len() {
            ...
            let report = evaluate(candidate.clone()).await;
    ...
    let minimized = evaluate(fixture.clone()).await;
...
mod tests {
    ...
    #[tokio::test]
    async fn reducer_prunes_irrelevant_structure_with_a_pure_predicate() {
        let fixture: NightFixture = serde_json::from_value(serde_json::json!({
            "seed": 7,
            "roster": [
                {"slot": "slot_1", "role": "doctor"},
                {"slot": "slot_2", "role": "mafia_goon"},
        ...
        .expect("synthetic minimizer fixture parses");
        let target = ReductionTarget::FailureClass(Some(FailureClass::SemanticExpectation));
        let (reduced, steps, report) =
            minimize_fixture_with(fixture, target, |candidate| async move {
                let preserves_failure = candidate
                    .actions
                    .iter()
                    .any(|action| action.action_id == "required");
                RunReport {
                    ok: !preserves_failure,
                    failure_class: preserves_failure.then_some(FailureClass::SemanticExpectation),
                    reason: preserves_failure.then(|| "synthetic semantic drift".to_string()),
                    game_id: None,
                    resolution_audited: None,
                    trace_count: None,
                    projection_audit_ok: None,
        ...
        assert!(!report.ok);
        assert_eq!(reduced.roster.len(), 2);
        assert_eq!(reduced.actions.len(), 1);
        assert_eq!(reduced.actions[0].action_id, "required");
        assert_eq!(
