...
use operator_proof::build_operator_projection_rebuild_audit_report;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let report = build_operator_projection_rebuild_audit_report(artifact_path, projection_report);
    let ok = report.ok;
    ...
    if ok {
        ...
        Err("projection rebuild artifact audit found drift".into())
    }
}
...
fn usage() -> String {
    "usage: audit_projection_rebuild_artifact [--output rebuild-report.json] <game-uuid>"
        .to_string()
}
