...
use operator_proof::build_operator_determinism_fuzz_report;

const DEFAULT_TEST_FILTER: &str = "replay_audit_and_rebuild_deterministically";
const AUDIT_THREAD_STACK_BYTES: &str = "8388608";
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let command_text = format!(
        "RUST_MIN_STACK={AUDIT_THREAD_STACK_BYTES} DATABASE_URL={} cargo test -p commands --test pipeline {} -- --ignored --nocapture",
        database_url, args.test_filter
    );
    let output = run_bounded_process(
        ProcessCommand::new("cargo")
            .env("DATABASE_URL", database_url)
            .env("RUST_MIN_STACK", AUDIT_THREAD_STACK_BYTES)
            .args([
                "test",
                "-p",
                "commands",
