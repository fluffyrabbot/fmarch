...
use uuid::Uuid;
use wire::{
    AckMsg, CapabilityGrant, ClientEnvelope, DayVoteOutcomeDelta, Hello, HostPhaseControl,
    PlayerInvestigationResult, PlayerNotification, ProjectionDelta, RejectCode, RejectMsg,
    ResolutionTraceInspectionReport, ServerEnvelope, ServerMsg, ThreadPage, VoteCountDelta,
    PROTOCOL_VERSION,
};
...
async fn player_notifications(
    ...
    Ok(Json(
        rows.into_iter().map(PlayerNotification::from).collect(),
    ))
}
