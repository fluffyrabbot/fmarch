...
use auth_http::{
    active_session_principal_and_globals, authenticate_token, bearer_token, env_i64,
    hash_session_token, internal_auth_error, require_active_enabled_account, require_global_admin,
    require_global_operator, unauthorized_account, unauthorized_session, unix_now_seconds,
    AuthHttpState,
};
...
pub fn router_with_state(state: ApiState) -> Router {
    ...
    let app = Router::new()
        ...
        .route("/ws", get(ws));
    let app = app.merge(media_routes).merge(auth_routes);
...
async fn optional_authenticated_community_member(
    ...
    let token = bearer_token(headers).ok_or_else(unauthorized_account)?;
    require_active_enabled_account(&state.auth, token)
        .await
        .map(Some)
}
