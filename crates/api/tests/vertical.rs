...
use wire::{
    ...
    ResolutionTraceInspectionReport, ServerEnvelope, ServerMsg, SlotLifecycle, ThreadPage,
    VoteTarget, PROTOCOL_VERSION,
};

fn router(pool: sqlx::PgPool) -> axum::Router {
    api::router(pool.clone()).merge(operator_api::router(pool))
}
...
#[sqlx::test(migrations = "../projections/migrations")]
async fn dev_global_admin_session_can_read_operator_status_only_when_dev_auth_enabled(
    pool: sqlx::PgPool,
) {
    let app = router_with_dev_auth(pool.clone());
    let game = Uuid::new_v4();
