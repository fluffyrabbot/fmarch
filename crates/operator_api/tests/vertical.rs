...
fn command_router(pool: sqlx::PgPool) -> Router {
    Router::new()
        .route("/commands", post(test_command))
        .with_state(CommandRouteState { pool })
}
...
async fn vertical_operator_index_is_host_audit_only(pool: sqlx::PgPool) {
    ...
    assert!(generated_shrink_go_no_go_html.contains("family_count: 29"));
    assert!(generated_shrink_go_no_go_html.contains("case_count: 58"));
