...
--
-- Classic (username + password) and WorkOS are coexisting first-class sign-in
-- methods on one platform principal. Detail rows are permanently bound to one
-- authentication method; auth_session is the single backend-owned app session.
...
ALTER TABLE public.auth_account
    ADD COLUMN method_id uuid NOT NULL;
...
-- auth_session is the single backend-owned app session. Dev and admin-grant
-- sessions intentionally have no authentication-method reference, but every
-- session has an explicit assurance and idle deadline.
ALTER TABLE public.auth_session
    ADD COLUMN authenticated_via_method_id uuid,
    ADD COLUMN idle_expires_at bigint NOT NULL,
    ADD COLUMN assurance text NOT NULL;
...
ALTER TABLE ONLY public.auth_session
    ...
    ADD CONSTRAINT auth_session_assurance_check CHECK (assurance = ANY (ARRAY['password'::text, 'external_sso'::text, 'dev'::text, 'admin_grant'::text])),
    ADD CONSTRAINT auth_session_idle_expiry_check CHECK (idle_expires_at > created_at);
...
-- Websocket tickets speak the current authentication-method vocabulary.
ALTER TABLE public.auth_websocket_ticket
    DROP CONSTRAINT auth_websocket_ticket_auth_kind_check;

ALTER TABLE public.auth_websocket_ticket
    ADD CONSTRAINT auth_websocket_ticket_auth_kind_check CHECK ((auth_kind = ANY (ARRAY['classic'::text, 'workos'::text, 'dev'::text, 'admin_grant'::text])));
