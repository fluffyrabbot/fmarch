-- 0024_event_stream_keys.sql — mirrored event-store DDL for the full schema.
-- Keep this catalog shape identical to eventstore migration 0003.
DO $$
BEGIN
...
CREATE TABLE public.event_stream_key_state (
    ...
    FOREIGN KEY (stream_id, active_epoch)
        REFERENCES public.event_stream_keys (stream_id, key_epoch)
        DEFERRABLE INITIALLY DEFERRED
);

CREATE INDEX event_stream_keys_wrap_kid_idx
    ON public.event_stream_keys (wrap_kid, stream_id, key_epoch);
...
CREATE FUNCTION public.event_direct_key_sentinel_immutable() RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER event_direct_key_sentinel_no_mutation
    BEFORE UPDATE OR DELETE OR TRUNCATE ON public.event_direct_key_sentinel
    FOR EACH STATEMENT EXECUTE FUNCTION public.event_direct_key_sentinel_immutable();

CREATE OR REPLACE FUNCTION public.event_stream_keys_guard_mutation() RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER event_stream_keys_guard
    BEFORE UPDATE OR DELETE ON public.event_stream_keys
    FOR EACH ROW EXECUTE FUNCTION public.event_stream_keys_guard_mutation();

CREATE TRIGGER event_stream_keys_truncate_guard
    BEFORE TRUNCATE ON public.event_stream_keys
    FOR EACH STATEMENT EXECUTE FUNCTION public.event_stream_keys_guard_mutation();

CREATE OR REPLACE FUNCTION public.event_stream_key_state_monotonic() RETURNS trigger AS $$
BEGIN
    IF TG_OP = 'DELETE' OR TG_OP = 'TRUNCATE' THEN
...
CREATE TRIGGER event_stream_key_state_guard
    BEFORE UPDATE OR DELETE ON public.event_stream_key_state
    FOR EACH ROW EXECUTE FUNCTION public.event_stream_key_state_monotonic();

CREATE TRIGGER event_stream_key_state_truncate_guard
    BEFORE TRUNCATE ON public.event_stream_key_state
    FOR EACH STATEMENT EXECUTE FUNCTION public.event_stream_key_state_monotonic();
