...
use commands::{audit_resolution_envelopes, operator_proof::build_operator_resolution_diff_report};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let audit = audit_resolution_envelopes(&pool, args.game).await?;
    let report = build_operator_resolution_diff_report(artifact_path, audit);
    let ok = report.ok;
    ...
    if ok {
        ...
        Err("resolution diff artifact found drift".into())
    }
}
...
fn usage() -> String {
    "usage: audit_resolution_diff_artifact [--output diff-report.json] <game-uuid>".to_string()
}
